...
import { DOCS, DOCS_CATEGORY } from "@/lib/docs-catalog";
      ...
      <div className="mx-auto max-w-5xl px-6 py-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Create, edit and publish docs.</p>
          </div>
          <button
        ...
        <div className="mt-6 rounded-lg border border-border bg-card p-4">
          <p className="text-xs text-muted-foreground mb-3">Quick create — pick a doc to edit:</p>
          <ul className="flex flex-wrap gap-1.5">
            {DOCS.map((doc) => (
              <li key={doc.slug}>
                ...
                >
                  <Plus className="h-2.5 w-2.5" /> {doc.name}
                </Link>
              </li>
            ))}
          </ul>
            ...
            <tbody>
              ...
              {docs.map((d) => (
                <tr key={d.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 text-muted-foreground font-mono text-xs">{d.slug}</td>
