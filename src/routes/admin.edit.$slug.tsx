...
function EditPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        ...
        .eq("category", DOCS_CATEGORY).eq("slug", slug).maybeSingle();
      if (data) {
        setDocId(data.id);
        setTitle(data.title);
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        category: DOCS_CATEGORY, slug, subcategory: doc.name,
  ...
  if (!doc) {
    return <SiteShell><div className="p-8 text-sm">Invalid doc.</div></SiteShell>;
  }
